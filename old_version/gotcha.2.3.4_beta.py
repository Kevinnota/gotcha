        ...
        if len(tip_no_node)!=0:
            for seq in tip_no_node:
                ...
                for node in used_nodes_it:
                    miss=0
                    anc_str=node_seq_dict[node][pos:(args.baitlength+pos)]
                ...
                miss_list=[]
                #print(node_seq_dict.keys())
                for key in node_seq_dict.keys():
                    if not "node" in key  :
                        continue
                    miss=0
                    anc_str=node_seq_dict[key][pos:(args.baitlength+pos)]
                    for nt in range(len(seq)):
                        if seq[nt].upper()!=anc_str[nt].upper():
            ...
            if(len(miss_dic)!=0):
                ...
                baitmer_list.append(anc_str)
                #print(needed_key[0]+" "+str(pos)+":"+str(args.baitlength+pos))
                c_baits=c_baits+1
